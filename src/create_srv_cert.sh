#!/usr/bin/env bash
        ...
        # $2 - pvt key [IN]
        # $3 - pfx [OUT]
        create_pkcs12.sh server/certs/server.crt server/private/server.key server/certs/server.pfx 

        echo "run dummy server on port 44330 to test tls"
        docker network 
        $osslweb s_server -key server/private/server.key -cert server/certs/server.crt -accept 44330 -www
